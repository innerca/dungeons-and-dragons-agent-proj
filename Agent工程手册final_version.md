...
© 2026 Xing Mingcheng。保留部分权利。

本手册（《Agent 工程手册（生产级·完整版·含深度思考题与参考答案）》）
由 Xing Mingcheng 与 DeepSeek 等 AI 模型协作创作完成。

许可条款

本作品采用知识共享 署名-非商业性使用-禁止演绎 4.0 国际许可协议（CC BY-NC-ND 4.0）
进行许可。您可以自由地：
...
- 署名 — 您必须给出适当的署名（Xing Mingcheng），提供指向本许可协议的链接，同时标明
  是否对原始作品作了修改。您可以用任何合理的方式来署名，但不得以任何方式暗示
  许可人为您或您的使用背书。
- 非商业性使用 — 您不得将本作品用于商业目的。
- 禁止演绎 — 如果您再混合、转换、或者基于本作品创作，您不可以分发修改后的作品。

完整许可文本：https://creativecommons.org/licenses/by-nc-nd/4.0/legalcode.zh-hans

免责声明

本手册内容基于截至 2026 年 4 月的行业实践与公开信息整理，仅供学习、研究、参考
之用。作者不对因使用或信赖本手册内容而导致的任何直接或间接损失承担责任。模型
性能、价格、框架 API 等时效性信息可能随时变更，请以官方最新文档为准。

手册中提及的公司、产品、服务名称均为其各自所有者的商标或注册商标，仅用于说明
性引用，不构成任何背书或关联关系。

贡献与反馈

本手册在 GitHub 上开源维护：
https://github.com/innerca/dungeons-and-dragons-agent-proj

欢迎通过 Issue 或 Pull Request 提出修正建议、补充案例或指出过时内容。所有贡献
者将被收录于致谢名单。

致谢

感谢 DeepSeek 及其他 AI 模型在知识检索、初稿生成、逻辑校验中的辅助。感谢所有
在生产实践中探索 Agent 工程的先行者，他们的公开分享构成了本手册的基石。

本手册将持续迭代。最新版本请访问：
https://github.com/innerca/dungeons-and-dragons-agent-proj/blob/main/Agent%E5%B7%A5%E7%A8%8B%E6%89%8B%E5%86%8Cfinal_version.md
