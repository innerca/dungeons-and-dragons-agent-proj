...
## 版本：v2.3 | 更新日期：2026-04

---

> **重要声明**：本手册是用于学习和建立 Agent 工程认知框架的参考材料，不是经过生产验证的标准答案。
> 文中的架构模式、指标口径、SLO、容量阈值、成本估算、性能数字与参数组合，包含大量基于 AI 辅助推演得到的示意值与假设值。
> 阅读时建议区分三层内容：
> 1. 原理与模式：可作为学习参考。
> 2. 参数与阈值：仅作起始假设或量级示意，不可直接用于生产决策。
> 3. 结论与推荐：只有在你的具体业务、模型、流量、基础设施和合规约束下完成验证后，才可视为可落地方案。
> 若将本手册用于团队分享、方案评审或对外传播，应明确标注其为“学习型参考手册”而非“生产规范”。
...
**数值阅读说明**：
- `[常见起始值]`：接近业内常见 starter config，可用于建立初始量级感。
- `[场景相关值]`：方向通常合理，但强依赖业务、模型、流量、基础设施，不宜视为通用标准。
- `[高风险伪精确值]`：看起来精确，但缺少统一行业基线或验证来源，只适合作为示意值阅读。
...
| 短期记忆 | Redis | 当前会话的原始对话（每轮 user+assistant） | 每次用户发送新消息时重置为 30 分钟 | 最近 20 轮 | 滑动窗口，超过 20 轮则压缩或丢弃最旧 |
...
**淘汰策略**：当长期记忆超过 200 条时，按分数从低到高删除，分数相同时按 LRU。当记忆数超限且仅剩强制存储项时，拒绝新记忆写入并触发运维告警。

**验证方法**：在 1000 个真实会话中比较原方案（LLM 打分）与新方案（规则+信号）的留存记忆质量。要求重要记忆召回率 ≥ 90%，成本降低约 99%（相比 LLM 打分方案）。
...
用户关闭浏览器后重新打开，如何快速恢复上下文？会话结束时将 Redis 中的短期记忆摘要写入 PostgreSQL 的 `sessions` 表；新会话启动时检查是否有未过期的摘要并预加载到 Redis。预加载的摘要作为短期记忆的初始上下文，与后续新对话共同构成短期记忆，而非覆盖。

#### 1.1.8 记忆冲突解决

同一用户在不同会话中给出矛盾偏好（如“我喜欢辣的”→“我不能吃辣”）。引入**时间衰减权重+冲突检测**：当新旧记忆内容矛盾（如情感极性相反或事实陈述冲突）且语义相似度 > 0.7 时，标记为冲突，触发 LLM 澄清或按时间戳决定优先级。冲突记录写入审计日志供产品分析。
...
“记住”不是一次写入，而是一条数据从生成到删除的完整生命周期。建议每条长期记忆至少包含以下字段：

| 字段 | 含义 | 示例 |
|------|------|------|
| `memory_id` | 记忆唯一 ID | `mem_9f...` |
| `source` | 来源 | `explicit_user`, `task_completion`, `inferred_preference` |
| `confidence` | 置信度 | `0.92` |
| `visibility` | 是否允许用户可见/可编辑 | `user_visible`, `system_only` |
| `expires_at` | 到期时间 | `2026-07-01T00:00:00Z` |
| `status` | 生命周期状态 | `candidate`, `active`, `suppressed`, `deleted` |

**推荐状态机**：
- `candidate`：新抽取但未稳定，暂不进入高优先检索。
- `active`：通过规则或用户确认，可参与上下文构建。
- `suppressed`：与新事实冲突、过期或被用户临时关闭，不删除但默认不检索。
- `deleted`：用户明确要求删除或命中合规策略，进入硬删除流程。

**治理原则**：
- 用户可见的偏好记忆必须支持“查看、纠正、删除、锁定不再更新”四个动作。
- `inferred_preference` 类型默认设置较短 TTL（如 14 天），除非被用户明确确认。
- 涉及健康、财务、身份、地理位置的敏感记忆默认 `system_only`，且不得用于广告、推荐等二次用途。
- 记忆进入 Prompt 前必须带 `source` 和 `updated_at`，避免模型把推断当事实。

**验证方法**：每周抽样 100 条被写入的长期记忆，检查“可解释来源率”≥ 95%、“用户纠错后 24 小时内生效率”≥ 99%、“被用户删除后检索残留率”≤ 0.1%。
...
- **全保留成本**：成本非线性增长（Transformer 复杂度 O(n²)），模型对长上下文中间部分的注意力权重下降（Lost in the Middle 效应），且越靠前的指令权重越低，极易导致越狱攻击（长文本覆盖短指令）。
...
> 数值标签：本节中的 `10900`、`4900`、各组成部分配额属于 `[场景相关值]`；它们适合作为预算拆分练习，不应被理解为行业标准。
...
| **合计输入** | **10900** | 输出预算 2000，总计约 12900（在 16K 内保留缓冲） |
...
| 平均 token 消耗 | 每请求总 token（输入+输出） | ≤ 12900（16K 配置，含缓冲） | 监控系统 |
...
> 数值标签：本节的超时、重试次数、退避策略大多属于 `[常见起始值]`；可作为默认参数起点，但上线前通常需要按依赖稳定性和用户容忍度重调。
...
**幻觉检测中间件**：在工具执行前，使用轻量级合法值缓存校验参数真实性。校验方法：对比用户历史实体缓存（如最近订单号列表）。
...
**去重表设计**：Redis，key = `idempotent:{tool_name}:{request_id}`，TTL = 1 小时。request_id 由调用方传入或系统自动生成（基于输入参数的哈希）。参数需经规范化（JSON key 排序、去除无关空格）后再哈希。若上次调用失败且失败原因非永久性（如网络超时），允许重新执行；永久性失败则直接返回失败结果。
...
Agent 与外部系统交互时，通常需要生成严格符合格式要求的数据（如调用工具的 JSON 参数、返回给前端的结构化卡片）。依赖自然语言描述格式容易导致解析失败、幻觉字段或格式错误。

**应用场景**：
- 工具调用的参数生成
- 提取用户对话中的结构化信息（如姓名、日期、金额）
- 生成固定格式的 API 响应
- 输出决策结果供下游系统消费

#### 2.8.2 JSON 模式配置

主流模型提供商均支持 JSON 模式（JSON Mode），通过 API 参数强制模型输出合法 JSON。

| 提供商 | 参数设置 | 说明 |
|--------|----------|------|
| OpenAI | `response_format: { "type": "json_object" }` | 需在 Prompt 中明确要求输出 JSON |
| Anthropic | 通过 `tool_choice` 强制调用工具或使用提示工程 | 无原生 JSON 模式，但工具调用天然为 JSON |
| 开源模型 | 使用 `grammar` 约束（如 llama.cpp）或 `guidance` 库 | 需推理引擎支持 |

**生产建议**：
- 对于关键参数提取，使用工具调用（Function Calling）代替纯 JSON 模式，因为工具调用经过模型微调，格式遵从性更高（准确率 ≥ 95% vs JSON 模式约 85%）。
- 若必须使用 JSON 模式，应结合**输出校验 + 重试机制**。

#### 2.8.3 输出校验与自修复

```python
def safe_json_parse(llm_output: str, expected_schema: dict, max_retries: int = 2):
    for attempt in range(max_retries + 1):
        try:
            ...
            # Schema 校验失败，将错误反馈给 LLM 进行自修复
            if attempt < max_retries:
                llm_output = llm_self_correct(llm_output, expected_schema)
    raise OutputValidationError("无法生成符合 Schema 的输出")
```

#### 2.8.4 结构化输出的 Token 效率优化

JSON 格式包含大量冗余字符（引号、花括号、逗号），约占总 token 的 20%-30%。优化策略：
- 使用更紧凑的格式（如 CSV 或自定义分隔符）替代 JSON（仅限内部系统通信）
- 采用 **JSON5** 或 **Hjson** 减少冗余（需双方支持）
- 对大规模数据返回，使用 `gzip` 压缩后传输，不计入 Token 成本

#### 2.8.5 结构化输出 SLI

| 指标 | 计算方式 | 目标值 |
|------|----------|--------|
| JSON 解析成功率 | 首次解析成功 / 总请求 | ≥ 95% |
| Schema 校验通过率 | 通过 Schema 校验 / 总请求 | ≥ 90% |
| 自修复成功率 | 经自修复后成功 / 首次失败请求 | ≥ 70% |

---

### 2.9 Workflow 编排

#### 2.9.1 为什么需要 Workflow 编排
...
> 数值标签：本节的 `chunk_size=512`、`overlap=64`、`top_k=30` 这类参数整体属于 `[常见起始值]`；不同文档类型的细分参数属于 `[场景相关值]`。
...
| 表格（Markdown/Excel） | 小表格独立成块；大表格按行切分，每块保留表头 | 0 | 每个块包含完整表头 + 若干数据行 |
...
- **策略表**：
    ...
    - **Markdown 表格**：**小表格独立成块；大表格按行切分，每块保留表头**。
...
> 数值标签：本节的成功率、P99、告警窗口整体属于 `[场景相关值]`；可作为 SLO 设计模板，但不是统一行业基线。
...
| 用户限流触发次数 | 每分钟内新触发限流的用户数 | ≤ 50 | > 200 持续 2 分钟 | P3 |
...
- alert: HighErrorBudgetBurn
  expr: (increase(http_requests_failed[1h]) / increase(http_requests_total[1h])) * 30 * 24 > 0.05
  for: 5m
  annotations:
    summary: "错误预算消耗过快"

- alert: CriticalSuccessRateDrop
  expr: (rate(http_requests_failed[5m]) / rate(http_requests_total[5m]) > 0.05) and (rate(http_requests_total[5m]) > 1)
  for: 2m
  labels:
    severity: P1
  annotations:
...
| 指标 | 计算方式（离线/在线均使用同一批回放样本进行完成率计算） | 合格阈值 | 不合格时的动作 |
|------|----------|----------|----------------|
...
线上 Agent 的问题如果不能复现，就无法稳定修复。每次请求都应记录一份**最小可复现实验信封**：

```json
{
  "trace_id": "tr_123",
  "model_version": "gpt-4.1-2026-04-15",
  "prompt_version": "prompt_v38",
  "toolset_version": "tools_v12",
  "workflow_version": "wf_v7",
  "retriever_version": "rag_v9",
  "feature_flags": ["use_semantic_cache", "new_router"],
  "temperature": 0,
  "seed": 42
}
```
...
**钉住原则**：
- Prompt、工具 Schema、Workflow、检索策略、重排模型、后处理规则都必须可独立版本化。
- 灰度期问题排查时，禁止直接使用“当前最新版本”回放，必须按原版本重放。
- 离线回放优先走 **VCR 录制回放** 或固定桩数据，避免上游工具状态变化造成假回归。
- 若线上请求使用了 Feature Flag，回放环境必须按原值恢复，否则复现结论无效。

**推荐流程**：
1. 用 `trace_id` 拉取完整请求信封和工具调用序列。
2. 在隔离环境固定 `temperature=0`、`seed`、版本号和测试数据快照。
3. 若问题无法复现，再逐项解除钉住，定位是模型漂移、工具漂移还是数据漂移。

**验证方法**：每月抽取 20 个真实线上故障单，要求基于请求信封的首轮复现成功率 ≥ 80%，三轮内定位根因成功率 ≥ 95%。
...
> 数值标签：本节的“2 元/天”“2000 元/天”以及 token 换算属于 `[高风险伪精确值]`；适合帮助理解预算思路，不适合直接采纳为成本标准。

- **单用户每日预算**：2 元人民币（约 1.8M token 的 GPT-4o-mini 或 30 万 token 的 GPT-4o），提前预警：当用户消耗达到预算 80% 时，返回提示“您今日的免费额度即将用完”。
- **全系统每日预算**：2000 元。
- **超出后动作**：
...
Agent 真实风险不在“会不会回答错”，而在“会不会拿着过大的权限做对了不该做的事”。建议把身份链显式拆成四层：

| 身份层 | 代表对象 | 典型字段 | 风险 |
|------|----------|----------|------|
| 终端用户身份 | 发起请求的人 | `user_id`, `tenant_id` | 越权访问他人数据 |
| Agent 运行时身份 | 当前 Agent 实例/工作流 | `agent_id`, `session_id` | 越权调用工具 |
| 连接器身份 | 代表用户访问 SaaS/数据库的令牌 | `connector_id`, `scope` | 长期令牌泄露 |
| 平台服务身份 | 后端服务账号 | `service_account` | 横向扩大事故范围 |

**治理规则**：
- 工具声明必须包含 `required_scopes`，例如 `calendar.read`, `calendar.write`，运行时按 scope 精确授权。
- 写操作默认使用 **On-Behalf-Of** 或短时访问令牌，避免把长期 refresh token 直接暴露给工具执行层。
- 连接器凭证统一加密存储于 Secrets Manager / KMS，应用只拿到临时解密结果，不落盘。
- 高风险工具使用 **JIT 凭证**：只有在用户确认后的 60 秒内签发一次性 token。
- 凭证轮换必须无感：旧 token 失效前，新 token 完成健康检查并灰度切换。

**审计要求**：
- 每次工具调用记录 `principal_type`, `principal_id`, `granted_scopes`, `credential_source`。
- 所有“权限不足”事件单独聚合，防止模型通过重试掩盖 RBAC 配置错误。

### 5.10 动作风险分级与确认策略

不是所有工具调用都该直接执行。建议在工具注册层定义 `risk_level`，由运行时统一执行确认策略：

| 风险等级 | 示例动作 | 默认策略 | 是否允许自动执行 |
|----------|----------|----------|------------------|
| `R0` 只读 | 查天气、查库存、读文档 | 直接执行 | 是 |
| `R1` 低风险写 | 新建草稿、保存偏好、生成草案 | 显示预览，可自动执行 | 是 |
| `R2` 中风险写 | 发邮件、创建日历、下单前填单 | 参数回显 + 一次确认 | 否 |
| `R3` 高风险动作 | 支付、退款、删除数据、修改权限 | 二次确认或人工审批 | 否 |

**执行细则**：
- `R2` 及以上动作必须向用户展示规范化摘要：对象、金额、时间、影响范围。
- `R3` 默认要求幂等键 `idempotency_key`，防止网络抖动或用户连点造成重复执行。
- 删除、支付、权限变更三类动作必须支持 `preview -> confirm -> execute` 三段式流程，不允许模型跳过预览。
- 若用户在对话中给出模糊授权（如“你看着办”），最多只放宽到 `R1`，不得据此执行 `R2/R3`。

**验证方法**：对所有写工具进行季度审查，要求 `risk_level` 标注覆盖率 100%，`R2/R3` 动作未经确认直执率为 0。
...
> 数值标签：本节的相似度阈值（如 `0.95`、`0.98`）和 TTL 大多属于 `[场景相关值]`；它们对 embedding 模型和 query 分布高度敏感。
...
| 创造性写作 | - | - | ❌ 不适用 | - |
| 工具调用决策（仅对幂等读操作启用，且需校验无副作用） | 30min | 0.96 | ⚠️ 谨慎使用 | 需校验工具调用副作用是否可重复 |
...
| 上游节点输出 | 按需 | 下游通过 `depends_on` 声明依赖，执行引擎自动注入摘要；上游输出经固定长度摘要模型（如按 800 token 截断）处理后传递 |
...
> 数值标签：本节的“接入时间 ≤ 2 小时”“事件到达率 ≥ 99.9%”“互操作成功率 ≥ 95%”属于 `[高风险伪精确值]`；更像目标值或示意值，而非行业共识。
...
| 视频（10 帧） | 假设抽帧分辨率为 512×512，每帧按图像 token 计算；实际需按真实分辨率计算 | 10 × 170 = 1700 token |
...
对于无法进行 SoM 预处理的场景（如本地桌面应用），直接使用微调后的 VLM 输出归一化坐标 `[x, y]`（归一化坐标 0-1000 符合 OpenAI 标准，或绝对像素坐标需附带 viewport 尺寸）。
...
// GUI 动作指令标准格式
{
  ...
  "target": {
    ...
    "bbox": [x1, y1, x2, y2], // 当无 element_id 时，归一化坐标 0-1000（符合 OpenAI 标准）或绝对像素坐标（需附带 viewport 尺寸）
    "text": "提交" // 可选，用于模糊匹配
  },
  "params": {
...
> 数值标签：本节的推理延迟、吞吐、显存占用、批处理参数整体属于 `[场景相关值]`；它们强依赖硬件、框架版本、量化方式和压测方法。
...
*测试条件：单卡 vLLM，batch size 动态，输入 512 / 输出 128 token。*
...
- 去重基于 `(instruction, user_context_hash)` 组合
- 规模：每周至少积累 2000 条高质量样本
- 多信号融合：用户显式确认 + 工具调用链闭合 + 后续对话无推翻，至少满足两个以上信号才算高置信度成功

**验证方法**：在保留的测试集上评估 SFT 后模型 vs 基线模型，要求任务完成率有统计显著提升（双尾 t 检验 p < 0.05），而非固定百分点。
...
| 隔离维度 | 生产推荐方案 | 成本优化方案 |
|----------|--------------|--------------|
| Redis（短期记忆/限流） | 每个租户独立 Redis 实例 | 逻辑分片（key 前缀 tenant_id），但无法确保容量隔离，需配合应用层配额 |
...
**应用层配额参考**：对于逻辑分片的 Redis，需在应用层实现租户级内存配额软拦截：
...
内存计数器使用 Redis Hash 结构 `tenant:mem:{tenant_id}`，字段为当前会话 ID 列表及各自大小。写入时 `HINCRBY` 增加，Key 过期时通过 **Keyspace Notifications** 监听回调减扣。
...
> 数值标签：本节的 `queue_depth > 200`、`wait_time_p95 > 2s`、`callback_lag > 60s` 等阈值属于 `[场景相关值]`；更像容量治理的示例报警线。

多租户系统不能只做“隔离”，还要做“受控退化”。核心目标是：高峰期宁可延迟可预期，也不要把所有租户一起拖死。

| 信号 | 阈值示例 | 运行时动作 | 用户感知 |
|------|----------|------------|----------|
| 网关排队深度 | `queue_depth > 200` | 开启入站限流，拒绝低优先级新请求 | 少量 429 |
| 推理队列等待 | `wait_time_p95 > 2s` | 低优租户切换小模型或缓存回答 | 质量轻降 |
| 工具回调积压 | `callback_lag > 60s` | 停止接收新的长任务，只保留轮询查询 | 异步任务变慢 |
| 单租户资源突刺 | `tenant_qps > tenant_quota * 2` | 仅对该租户熔断，不影响全局 | 该租户限流 |

**工程要求**：
- 所有异步队列必须支持 `DLQ (Dead Letter Queue)`，连续失败超过阈值的消息不可无限重试。
- 请求进入执行层前做 **Admission Control**：若预计 token 或工具成本超出系统当前容量，提前拒绝或降级。
- 对用户已取消、页面已关闭、超时已无意义的请求执行**协作式取消**，不要占着 GPU/工具槽位继续跑。
- 长任务队列按租户和任务类型分桶，避免“一个大租户的批量导出任务”挤占全部 worker。

**容量规划参考**：
- 高峰期目标队列等待时间 `queue_wait_p95 ≤ 1s`。
- 任何单租户占用的并发槽位不应超过总池子的 30%。
- DLQ 消息 24 小时内必须人工清理或自动归档，不能无限堆积。
...
1. 一个会话持续了 7 天（用户每天来聊几句），7 天内 Agent 版本迭代了 3 次，这个用户还在用最老的版本，而且那个版本有一个已知的安全漏洞。你会强制迁移这个会话吗？如何平衡安全与用户体验？
2. 灰度回滚自动化中，如果因流量自然波动（非版本问题）导致 P99 短暂飙升，触发自动回滚，反而造成服务不稳定。你如何设计回滚阈值和持续时间来避免误触发？
3. 灾难恢复演练中，RPO < 5 分钟意味着可能丢失最后 5 分钟的数据。对于金融、医疗等强一致性场景，这个 RPO 是否可接受？如果不接受，你会如何改造架构？
...
> 数值标签：本节的 TTFT、token 间隔、吞吐等指标属于 `[场景相关值]`；对低延迟产品有参考价值，但不能视为所有 Agent 系统的统一标准。
...
|------|------------|----------|--------------|
| 记忆管理 | 重要记忆召回率 ≥ 90%（相比 LLM 打分方案成本降低 ≥ 99%） | 1000 个真实会话，2 名标注员独立评估，Fleiss' Kappa ≥ 0.7 | 补充更多规则 |
| Workflow | 并行加速比 ≥ 1.5，模式选择准确率 ≥ 90% | 在 200 个不同任务实例上执行并统计 | 优化调度器或 LLM 决策 prompt |
| 一致性校验 | 同一批采样会话，离线与在线完成率差值 ≤ 5%；ρ ≥ 0.7 | 每周报告 | 更新测试集或指标定义 |
| 多模态成本 | 相比无降级基线，单次请求成本降低 ≥ 60% | A/B 测试验证，任务完成率下降 ≤ 2% | 调整默认配置或用户等级配额 |
| 数据飞轮 | 每周采样 ≥ 5000 条有效轨迹；SFT 任务完成率绝对提升 ≥ 2% 且双尾 t 检验 p < 0.05 | 连续 4 周达标 | 检查采样逻辑或数据质量 |
| 私有化部署 | 20 QPS 下，P99 延迟 ≤ 200 ms/100 token，持续 1h 无 OOM | 压测达到 100 QPS 且满足指标 | 扩容或调整量化级别 |
| 推理效率 | 缓存命中率 ≥ 30%；Token 节省率 ≥ 20%（计算公式见正文） | A/B 测试 | 调整相似度阈值或缓存策略 |
| 多智能体 | MAS 任务完成率 ≥ 85%；混沌工程中检查点恢复成功率 ≥ 99.9% | 500 工单验证完成率；1000 次崩溃注入验证恢复 | 优化状态图设计或恢复逻辑 |
| 通信协议 | MCP 简单工具接入 ≤ 2h；跨厂商互操作成功率 ≥ 95% | 20 个工具接入计时取平均 | 完善适配器或命名空间方案 |
| GUI Agent | 在 WebArena 上，元素定位成功率 ≥ 90%，端到端成功率 ≥ 40% | WebArena 基准测试 | 优化 SoM 预处理或 VLM 选择 |
| 端云协同 | 端侧处理占比 ≥ 40%；冲突率 ≤ 1%（冲突数/总同步事件） | 线上统计 | 调整分流阈值或 CRDT 规则 |
| 多租户与版本迁移 | 高负载下其他租户 P99 延迟增加 ≤ 20%；版本迁移成功率 ≥ 95% | 压测 + 迁移测试 | 扩容或优化路由 |
| 流式响应 | 在典型生产配置下，TTFT P95 ≤ 1.2s，流中断率 ≤ 2% | 线上持续监控 | 优化首 Token 生成路径 |
