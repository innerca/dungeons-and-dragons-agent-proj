...
**额外声明**：
手册中部分高阶主题（如多模态、数据飞轮、私有化部署）在当前的个人项目中尚未完全落地验证。我只是投喂了相关的材料后进行思维拓扑，要求 AI 进行批判验证，不代表我已经掌握相关内容。

**迷你版定位**：
- 这是一份**可独立阅读的生产速查手册**，覆盖单 Agent 落地的最小闭环。
- 它优先保留“默认做法、关键阈值、失败兜底、验证方法”，刻意省略部分高阶展开。
- 若你正在做 MAS、GUI Agent、协议标准化、端云协同等复杂主题，请把本手册视作入口，再去看完整版对应章节。

**数值阅读说明**：
- `[常见起始值]`：接近常见 starter config，可用于建立工程量级感。
- `[场景相关值]`：方向通常合理，但强依赖模型、业务和基础设施，不宜视为通用标准。
- `[高风险伪精确值]`：看起来很具体，但缺少验证来源，只适合作为示意值。

## 目录

- [0. 总览：Agent 请求全链路](#0-总览agent-请求全链路)
- [1. 输入侧：上下文管理](#1-输入侧上下文管理)
  - [1.1 三层记忆架构](#11-三层记忆架构)
  - [1.2 Token 预算分配](#12-token-预算分配)
  - [1.3 上下文压缩触发条件与算法](#13-上下文压缩触发条件与算法)
  - [1.4 提示词优化指标](#14-提示词优化指标)
- [2. 工具调用体系](#2-工具调用体系)
  - [2.1 工具 Schema 标准](#21-工具-schema-标准)
  - [2.2 工具调用失败处理矩阵](#22-工具调用失败处理矩阵)
  - [2.3 状态保存与恢复](#23-状态保存与恢复)
  - [2.4 Workflow 编排](#24-workflow-编排)
- [3. 可观测性与告警](#3-可观测性与告警)
  - [3.1 SLI 目标与告警阈值](#31-sli-目标与告警阈值采用错误预算理念)
  - [3.2 告警规则示例](#32-告警规则示例prometheus-语法)
- [4. 限流与降级](#4-限流与降级)
  - [4.1 限流配置](#41-限流配置令牌桶)
  - [4.2 降级策略](#42-降级策略)
- [5. 安全与合规](#5-安全与合规)
  - [5.1 Prompt 注入检测](#51-prompt-注入检测)
  - [5.2 成本熔断](#52-成本熔断)
  - [5.3 审计日志](#53-审计日志)
  - [5.4 数据边界与事故响应](#54-数据边界与事故响应)
  - [5.5 工具执行安全沙箱](#55-工具执行安全沙箱)
- [6. 测试与评估](#6-测试与评估)
  - [6.1 离线测试集结构](#61-离线测试集结构)
  - [6.2 评估指标](#62-评估指标)
  - [6.3 回归测试流程](#63-回归测试流程)
  - [6.4 离线-在线评测一致性校验](#64-离线-在线评测一致性校验)
- [7. 部署与灰度](#7-部署与灰度)
  - [7.1 灰度放量计划](#71-灰度放量计划)
  - [7.2 自动回滚条件](#72-自动回滚条件)
- [8. 流式响应处理](#8-流式响应处理)
  - [8.1 流式输出的工程要求](#81-流式输出的工程要求)
  - [8.2 流式响应的 SLI 扩展](#82-流式响应的-sli-扩展)
  - [8.3 断线重连与消息续传](#83-断线重连与消息续传)
- [9. 工程治理补充](#9-工程治理补充)
  - [9.1 模型版本管理与 A/B 测试](#91-模型版本管理与-ab-测试)
  - [9.2 身份、权限与凭证治理](#92-身份权限与凭证治理)
  - [9.3 动作风险分级与确认策略](#93-动作风险分级与确认策略)
  - [9.4 版本钉住与问题回放](#94-版本钉住与问题回放)
  - [9.5 异步任务与回调](#95-异步任务与回调)
  - [9.6 运行时治理与背压控制](#96-运行时治理与背压控制)
  - [9.7 成本优化策略](#97-成本优化策略)
  - [9.8 可解释性与调试支持](#98-可解释性与调试支持)
  - [9.9 边缘情况处理](#99-边缘情况处理)
  - [9.10 延伸阅读导航](#910-延伸阅读导航)
- [10. RAG 精确分块策略](#10-rag-精确分块策略)
  - [10.1 分块前置处理](#101-分块前置处理)
  - [10.2 分块策略](#102-分块策略递归分块--语义边界)
  - [10.3 语义边界增强](#103-语义边界增强可选)
  - [10.4 分块后处理](#104-分块后处理)
  - [10.5 检索与重排](#105-检索与重排)
- [11. 多模态 Agent API 规范](#11-多模态-agent-api-规范)
  - [11.1 支持的输入模态](#111-支持的输入模态)
  - [11.2 多模态 API 请求格式](#112-多模态-api-请求格式)
  - [11.3 多模态工具定义](#113-多模态工具定义)
  - [11.4 多模态响应格式](#114-多模态响应格式)
  - [11.5 音频/视频预处理参数](#115-音频视频预处理参数)
  - [11.6 多模态 Agent 的上下文预算分配](#116-多模态-agent-的上下文预算分配)
  - [11.7 多模态成本控制与降级](#117-多模态成本控制与降级)
- [12. Agent 数据飞轮与模型优化](#12-agent-数据飞轮与模型优化)
  - [12.1 为什么需要数据飞轮](#121-为什么需要数据飞轮)
  - [12.2 数据采集与采样策略](#122-数据采集与采样策略)
  - [12.3 训练数据构造：监督微调（SFT）](#123-训练数据构造监督微调sft)
  - [12.4 偏好学习（DPO）](#124-偏好学习dpo)
  - [12.5 数据飞轮指标](#125-数据飞轮指标)
- [13. 私有化部署与开源模型适配](#13-私有化部署与开源模型适配)
  - [13.1 为什么需要私有化适配](#131-为什么需要私有化适配)
  - [13.2 开源模型推理性能基线](#132-开源模型推理性能基线)
  - [13.3 工具调用格式适配](#133-工具调用格式适配)
  - [13.4 显存与并发管理](#134-显存与并发管理)
- [14. 多租户架构与版本迁移](#14-多租户架构与版本迁移)
  - [14.1 多租户隔离要求](#141-多租户隔离要求)
  - [14.2 版本迁移与平滑升级](#142-版本迁移与平滑升级)
- [附录：整体验证方法总结](#附录整体验证方法总结)
...
长期记忆最好不要只有“写入/不写入”两种状态。最小可行设计建议包含：

| 字段 | 作用 |
|------|------|
| `source` | 标识来自用户显式要求、任务完成还是模型推断 |
| `confidence` | 区分事实、偏好、猜测 |
| `status` | `candidate/active/suppressed/deleted` |
| `expires_at` | 控制推断类记忆自动过期 |

**默认规则**：
- 模型推断出的偏好默认短 TTL，例如 14 天。
- 用户显式要求“记住”的内容进入 `active`，且默认 `user_visible`。
- 用户明确纠正或删除后，旧记忆进入 `suppressed` 或 `deleted`，不能继续参与检索。

### 1.2 Token 预算分配

> 数值标签：本节中的 `10900`、`4900` 及各组成配额属于 `[场景相关值]`；它们适合帮助理解预算拆分，不应被视为标准答案。
...
> 数值标签：本节的超时、重试次数大多属于 `[常见起始值]`；上线前通常还要结合依赖质量和用户耐心调整。
...
> 数值标签：本节的成功率、P99 和告警阈值整体属于 `[场景相关值]`；它们更像模板，不是统一行业基线。
...
> 数值标签：本节的预算金额和 token 换算属于 `[高风险伪精确值]`；只建议用来理解成本治理思路。
...
> 数值标签：本节的 TTFT、token 间隔、吞吐指标属于 `[场景相关值]`；它们很适合拿来做学习标尺，但不应直接当作生产承诺。
...
流式响应不能假设前端永不掉线。服务端至少应支持以下机制：

- 每条 SSE 事件带递增 `message_seq` 或 `event_id`。
- 客户端断线重连时上报 `last_event_id`，服务端从该序号之后继续推送。
- 工具调用状态、文本增量、错误事件都进入同一条可回放事件流，避免“文字续上了，但工具状态丢了”。
- 事件缓存 TTL 建议 ≥ 5 分钟；超过 TTL 则回退为“整段结果 + 当前最新状态”的补发模式。
...
最常见的生产事故不是模型“回答笨”，而是 Agent 拿了过大的权限。最小治理要求：

- 工具声明 `required_scopes`，例如 `calendar.read`、`calendar.write`，运行时按 scope 授权。
- 连接器令牌统一放在 KMS / Secrets Manager，加密存储，不落盘。
- 写操作优先使用短时访问令牌或 On-Behalf-Of 令牌，不把长期 refresh token 暴露给执行层。
- 跨用户数据访问必须要求显式授权，且审计日志记录 `authorized=true/false`。

### 9.3 动作风险分级与确认策略

建议在工具注册层定义 `risk_level`：

| 等级 | 示例 | 默认策略 |
|------|------|----------|
| `R0` | 查询、检索、总结 | 直接执行 |
| `R1` | 保存草稿、写偏好 | 预览后可自动执行 |
| `R2` | 发邮件、建日历、提交表单 | 参数回显 + 用户确认 |
| `R3` | 支付、删除数据、改权限 | 二次确认或人工审批 |

删除、支付、权限变更三类动作建议统一使用 `preview -> confirm -> execute` 三段式流程。

### 9.4 版本钉住与问题回放

如果线上问题不能稳定复现，排查会非常低效。每次请求最少记录：
...
- 回调消息必须带 `trace_id` 和 `task_id`，便于排障与幂等去重。
- 长任务如果超过前端页面生命周期，应支持“稍后回来继续看”，而不是只依赖当前连接。

### 9.6 运行时治理与背压控制

> 数值标签：本节虽然数值较少，但默认阈值思路整体属于 `[场景相关值]`；真实系统通常需要用压测和线上回放来定线。

当流量上来时，系统要先学会“稳住”，再谈“变聪明”：

- 网关排队深度过高时，先拒绝低优先级请求，而不是把所有请求一起拖慢。
- 单租户突发流量应只熔断该租户，避免全局连带故障。
- 异步队列要有 DLQ，连续失败的消息不能无限重试。
- 对用户已取消或超时已无意义的任务做协作式取消，释放 GPU 和工具槽位。
...
- 面向工程排障的 trace 可以比面向用户的解释更详细，两者不要混用。
...
以下主题在迷你版只给最小结论，实际工程设计建议去完整版深读：

| 主题 | 迷你版处理方式 | 完整版对应方向 |
|------|----------------|----------------|
| 推理效率优化 | 只保留缓存与模型路由 | 语义缓存、自适应推理、投机性工具调用 |
| 多智能体 | 迷你版不展开 | MAS 架构模式、状态图、上下文隔离 |
| 协议标准 | 迷你版不展开 | MCP、A2A、AG-UI |
| GUI Agent | 迷你版不展开 | SoM 定位、视觉操作、安全沙箱 |
| 端云协同 | 只保留私有化适配 | 端侧 Agent、端云分流、跨地域部署 |
...
> 数值标签：本节的 `chunk_size=512`、`overlap=64` 等参数整体属于 `[常见起始值]`；特殊文档的差异化参数属于 `[场景相关值]`。
...
> 数值标签：本节的延迟、吞吐、显存占用属于 `[场景相关值]`；它们高度依赖硬件、框架版本、量化方式和 batch 策略。
...
本手册基于作者与 AI 协作整理，所有参数、阈值与策略均需结合实际生产环境验证后使用。
